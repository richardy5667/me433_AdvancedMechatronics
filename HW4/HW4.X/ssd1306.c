...
void ssd1306_drawLetter(unsigned char x, unsigned char y, unsigned char letter){
    unsigned char color, dx, dy, new_y;
    for(dx=0;dx<=4;dx++){        
        for(dy=0;dy<=7;dy++){      
            new_y=y+dy;
            color=(ASCII[letter-0x20][dx]>>dy)&1;
            ssd1306_drawPixel(x,new_y,color);
        }
        x=x+1;
    }
}

void ssd1306_drawMessage(unsigned char x, unsigned char y, unsigned char message[]){
    int s=0;
    while(message[s]!=0){
        ssd1306_drawLetter(x,y,message[s]);
        x=x+5;
        s++;
        if(x>124){
            x=0;
